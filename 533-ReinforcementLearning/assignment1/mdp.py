...
def load_args():
  ...
  parser.add_argument('-t', '--timestep', default=0, help='horizon length, discarded if discount provided', required=False)
  parser.add_argument('-g', '--gamma', default=0, help='discount factor', required=False)
  parser.add_argument('-i', '--input', default='MDP1.txt', help='input file with MDP description', required=True)
...
if __name__ == '__main__':
    args = load_args()
    grid, actions = load_data(args.input)
    mdp = MDP(args, grid, actions)
    Value = mdp.Q()
    Policy = mdp.policy()

    print "Value for the last run was : {}\n\nPolicy : {}\n**************************************".format(Value, Policy)
