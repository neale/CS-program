  ...
  while(1) {
    ...
    break;
      ...
      case SR_RANDOM_DELAY: {
        ...
        TCCR0 &= ~(1 << CS00) & ~(1 << CS01) & ~(1 << CS02);                          //normal mode, clock disabled
        ...
        do {
        while(!(TIFR & (1 << TOV0))) {}; //spin till TC0 OVF flag set
    ...
    case SR_TIMING_USER: {
      ...
      TCCR1A &= ~(1 << WGM11) & ~(1 << WGM10);                 //disable all output compare pins and set WGM1{1,0} to 0
      ...
      TCNT1   = 0x00;                                    // initialize TCNT counter to 0
