...
void send_lcd(uint8_t cmd_or_char, uint8_t byte){
  ...
  while (bit_is_clear(SPSR,SPIF)){}   //wait till byte is sent out
  strobe__lcd();                       //strobe the LCD enable pin
#else //4-bit mode
...
void lcd_init(void){
  ...
  LCD_PORT = 0x30; strobe__lcd(); _delay_ms(8);  //function set,   write lcd, delay > 4.1ms
  LCD_PORT = 0x30; strobe__lcd(); _delay_us(200);//function set,   write lcd, delay > 100us
  LCD_PORT = 0x30; strobe__lcd(); _delay_us(80); //function set,   write lcd, delay > 37us
  LCD_PORT = 0x20; strobe__lcd(); _delay_us(80); //set 4-bit mode, write lcd, delay > 37us
