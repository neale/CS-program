...
void put_header(int sock, char * header) {
    ...
    if (err < 0) {
        fprintf(stderr, "otp_dec_d: could not write header to socket..");
    }
}
/*
...
void put_header_msg(char * msg, char * crypt_key, char * data, char * cryptxt, char * header, int size, int sock) {
    ...
    while (err != 0) {
        ...
        if (err < 0) {
            fprintf(stderr, "otp_dec_d: Could not write to socket..\n");
        }
    }
} 
...
void parse(char * msg, int msg_size, int * data_size, int * key_size, char * key, char * data) {
    ...
    for (i = 0; i < msg_size; i++) {
        ...
        if (!isvalid) {
            fprintf(stderr, "otp_dec_d: input must be [A - Z] or ' ' \n");
            exit(-1);
        }
    }
}
...
void spawn (Host *host, int fsock) {
    ...
    if(pid == 0){ //child process
        ...
        if(!isvalid){
            printf("otp_dec_d: network crosstalk not permitted..\n");
            exit(-1);
        }
        ...
        if(key_size < data_size){
            printf("otp_dec_d: key must be longer than <%d>\n", data_size);
            exit(-1);
        }
...
void connect_out(Host * host) {
    ...
    if (fsock < 0) {
        fprintf (stderr, "otp_dec_d: could not connect..\n");
    }
    spawn(host, fsock);
}
...
int main(int argc, char ** argv) {
    ...
    if (sigaction(SIGCHLD, &sig, NULL) == -1) {
        fprintf(stderr, "otp_dec_d: uncaught signal");
    }
