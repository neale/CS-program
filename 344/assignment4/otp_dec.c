...
int connect_sock(char **data) {
    ...
    if (sock < 0) {
        fprintf(stderr, "opt_dec: Could not open socket..");
        return -1;
    }
    ...
    if (host == NULL) {
        fprintf(stderr, "opt_dec: Could not connect to host..");   
        return -1;
    }
    ...
    if (connect(sock, (struct sockaddr *) &host_addr, sizeof(host_addr)) < 0) {
        fprintf(stderr, "otp_dec: Could not connect to port..\n");
        exit(-1);
    }
    return sock;
}
...
void put_header(int sock, char * header) {
    ...
    if (err < 0) {
        fprintf(stderr, "otp_dec: could not write header to socket..");
    }
}
...
void put_msg(char * msg, char * header, char ** args, int sock) {
    ...
    while (err != 0) {
        ...
        if (err < 0) {
            fprintf(stderr, "otp_dec: Could not write to socket..\n");
        }
    }
} 
...
void get_msg(char * msg, int size, int sock) {
    ...
    while (bytes_read < size) {
        ...
        if (bytes < 0) {
            fprintf(stderr, "otp_dec: could not read message..");
        }
        buf[bytes] = '\0';
        strcat(msg, buf);
        bytes_read += bytes;
