...
Assignment: 4
Course: CS 344
Executable: otp_dec_d
File Name: otp_dec_d.c
Purpose: Decryption daemon
...
//packet transmission
void create_dec_packet(char dec_packet[], char dec_header[], char ciphertext[]);
...
void create_plaintext(char key[], char ciphertext[], char plaintext[], int input_size);
...
void setup_server_socket(MSOCK_SERVER *SOCK_SERVER, int argc, const char *argv[]){
  ...
  if (argc < 2) { //make sure that port number is provided.
      fprintf(stderr,"OTP_DEC_D: ERROR, no port provided\n");
      exit(1);
  }
...
void execute(MSOCK_SERVER *SOCK_SERVER, int newsockfd){
  ...
  if(pid == 0){ //child process
    ...
    if(!valid){
      printf("OTP_DEC_D: PROGRAM MISMATCH! otp_dec_d cannot connect to otp_enc\n");
      exit(1);
    }
    ...
    if(key_size < input_size){
      printf("OTP_DEC_D: ERROR KEY TOO SHORT!\n");
      exit(1);
    }

    create_plaintext(key, input, plaintext, input_size);
    create_dec_packet(dec_packet, dec_header, plaintext);
    send_header(dec_header, newsockfd);
    send_packet(dec_packet, newsockfd);
...
void recv_packet(char packet[], int packet_size, int sockfd){
  ...
  if (n < 0) error("OTP_DEC_D: ERROR reading from socket");
...
void parse_packet(char packet[], int packet_size, int *input_size, int *key_size, char key[], char input[]){
  ...
  for(i=0; i < packet_size; i++){
    ...
    if(! is_legal(packet[i])){
      printf("OTP_DEC_D: NOT LEGAL INPUT\n");
      exit(1);
    }
  } //forloop
}
...
void create_dec_packet(char dec_packet[], char dec_header[], char ciphertext[]){
  int i, dec_packet_size;
  char dec_packet_size_s[10];

  dec_packet_size = strlen(ciphertext); //find packet_size
  snprintf(dec_packet_size_s, 10, "%i", dec_packet_size); //string format packet_size

  for(i=0; i< 10; i++){
    if(!isdigit(dec_packet_size_s[i])){ //isdigit is part of ctype.h
      //padding is necessary to maintain accuracy of read/write ops in socket
      dec_packet_size_s[i] = ' ';
    }
    dec_packet_size_s[10] = '\0'; //string termination.
  }
  //string format the ID bytes and the packet_size into the header.
  snprintf(dec_header, 102, "$;%s", dec_packet_size_s);
  //string format the encrypted plaintext into the packet.
  snprintf(dec_packet, 100000, "%s",ciphertext);
}
  ...
  */
  ...
  for(i=0; i< (input_size); i++){
    ...
    if((ciphertext[i] != ' ') && (key[i] != ' ')){
      plaintext[i] = mod(((ciphertext[i] - 65) - (key[i] - 65)), 27);
      if(plaintext[i] == 26){
        plaintext[i] += 6; //get to ascii value for a SPACE.
      } else if(plaintext[i] != 26){
        plaintext[i] += 65; //lots of ascii shifting
      }
    }

    if((ciphertext[i] == ' ') && (key[i] != ' ')){
      plaintext[i] = mod(((ciphertext[i] - 6) - (key[i] - 65)), 27); //lots of ascii shifting
      if(plaintext[i] == 26){
        plaintext[i] += 6; //get to ascii value for a SPACE.
      } else if(plaintext[i] != 26){
        plaintext[i] += 65; //lots of ascii shifting
    ...
    if((ciphertext[i] != ' ') && (key[i] == ' ')){
      plaintext[i] = mod(((ciphertext[i] - 65) - (key[i] - 6)), 27);
      if(plaintext[i] == 26){
        plaintext[i] += 6; //get to ascii value for a SPACE.
      } else if(plaintext[i] != 26){
        plaintext[i] += 65; //lots of ascii shifting
    ...
    if((ciphertext[i] == ' ') && (key[i] == ' ')){
      plaintext[i] = mod(((ciphertext[i] - 6) - (key[i] - 6)), 27);
      if(plaintext[i] == 26){
        plaintext[i] += 6; //get to ascii value for a SPACE.
      } else if(plaintext[i] != 26){
        plaintext[i] += 65; //lots of ascii shifting
...
plaintext[(input_size)] = '\0';
}

/*Special mod function needed to correctly take the mod of a negative number.*/
char mod (char a, char b){
   int ret = a % b;
   if(ret < 0)
     ret+=b;
